...
export default class DeleteRecipe extends Component {
    ...
    async onDelete(e) {
        ...
        const response = await axios.delete(`http://localhost:4000/recipes/delete/${recipeID}`)
        .then((res) => {
            console.log(res.data)
        }).catch((error) => {
            console.log(error)
        ...
        return (
            <div className="wrapper">
                ...
                <FormGroup onSubmit={this.onUpdate}>
                    ...
                    <div className="form-group">
                        <Button type="submit" value="Create recipe" variant="contained" onClick={this.onDelete} style={{ backgroundColor: '#f1b341', color: 'white' }}> Delete Recipe </Button>
                    </div>
                </FormGroup>
                </Box>
            </div>
